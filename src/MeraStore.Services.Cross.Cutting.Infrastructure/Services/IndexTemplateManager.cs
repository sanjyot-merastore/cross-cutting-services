        ...
        foreach (var pattern in _indexPatterns)
        {
            ...
            foreach (var sourceIndex in matchingIndices)
            {
                ...
                if (!createResp.IsValidResponse)
                    throw LoggingException.LogReindexingFailed($"Failed to create temp index '{tempIndex}': {createResp.ElasticsearchServerError}");
                ...
                if (!reindexResponse.IsValidResponse)
                    throw LoggingException.LogReindexingFailed($"Reindexing failed for index '{sourceIndex}': {reindexResponse.ElasticsearchServerError}");
                ...
                if (!deleteResponse.IsValidResponse)
                    throw LoggingException.LogReindexingFailed($"Failed to delete temp index '{tempIndex}': {deleteResponse.ElasticsearchServerError}");
